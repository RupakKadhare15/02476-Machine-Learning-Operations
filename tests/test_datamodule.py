...
def temp_data_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        dummy_data = pd.DataFrame({
            'comment_text': ['test1', 'test2', 'test3'],
            'toxic': [0, 1, 0]
        })
    ...
    def test_full_pipeline_with_mocks(self, temp_data_dir):
        ...
        with patch('transformers.AutoTokenizer.from_pretrained') as mock_tokenizer_class:
            ...
            def mock_tokenize(text, **kwargs):
                ...
                tokens = [101] + [i % 1000 for i in range(min(len(text.split()), kwargs.get('max_length', 128)-2))] + [102]
                padded = tokens + [0] * (kwargs.get('max_length', 128) - len(tokens))
                
                return {
                    'input_ids': torch.tensor([padded]),
                    'attention_mask': torch.tensor([[1] * len(tokens) + [0] * (kwargs.get('max_length', 128) - len(tokens))])
                }
